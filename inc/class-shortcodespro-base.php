	...
	class MV_ShortcodesPro_Base
	{
		...
		*/
		...
		function get_buttons()
		{
			...
			set_transient('sp.get.buttons',$results,MV_SHORTCODES_PRO_TRANSIENT_EXPIRE);
				...
				case 'attributes':
						...
						if ( isset($meta['totalattr'][0]) && $meta['totalattr'][0] > 0 && isset($meta['attrvals'][0]))
						{
							...
							foreach ($fields as $field) 
							{
								...
								<a title="Edit Attribute" class="edit button thickbox" href="admin-ajax.php?action=scpeditattribute&width=640&id='.$field['slug'].'">'.__('Edit',MV_SHORTCODES_PRO_DOMAIN).'</a> 
								<a title="Delete Attribute" class="delete button" href="#">'.__('Delete',MV_SHORTCODES_PRO_DOMAIN).'</a>'.$hidden.'</td></tr>';
							}
		...
		*/
		...
		function field_select($field,$meta=null)
		{
			...
			foreach ($field['options'] as $option) 
			{
				...
				if ( !is_array($option))
				{
					$selected = ( $field['value'] == $this->slug_string($option)) ? ' selected="selected"' : '';
					echo '<option  value="'.$this->slug_string($option).'" '.$selected.'>'.$option.'</option>';
				}
				else
				{
